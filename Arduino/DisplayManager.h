 ...
 * - Startup and error screens
 * - Customizable layouts
 * 
 * Display Layout:
 * - Top: System status and session ID
 * - Middle: Current sensor readings (V, A, W, Hz, °C)
 * - Bottom: ML prediction and threat status
 * - Status bar: WiFi, charging, alerts
 * 
 * Usage:
 * 1. Initialize with DisplayManager::init()
 * 2. Update display with DisplayManager::updateDisplay()
 * 3. Show special screens with DisplayManager::showStartupScreen(), etc.
 */
...
void DisplayManager::showStartupScreen() {
  ...
  for (int i = 0; i < 3; i++) {
    _drawCenteredText(80, "Initializing" + String(".").repeat(i + 1), COLOR_YELLOW, 1);
    delay(500);
  }
